...
target_object_height_mm = 100 # NEED TO MODIFY PER OBJECT
image_height_pixels = 240.0 # QVGA
sensor_h_mm = 2.952 # For OV7725 sensor - see datasheet.
offest_mm = 100.0 # Offset fix...
def blob_to_distance(blob): # r == (x, y, w, h) -> r[3] == h
    return ((lens_mm * average_head_height_mm * image_height_pixels) / (blob.h * sensor_h_mm)) - offest_mm
...
while(True):
    ...
    for blob in img.find_blobs(thresholds, pixels_threshold=100, area_threshold=100, merge=True):
        if blob.code() == 3: # r/g code == (1 << 1) | (1 << 0)
            ...
            # get distance
            obj_distance = blob_to_distance(blob)
            break # only get one blob?
